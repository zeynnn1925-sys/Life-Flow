...
import { Plus, Trash2, Clock, CheckCircle2, ChevronLeft, ChevronRight, Calendar as CalendarIcon, List } from 'lucide-react';
...
export default function DailySchedule() {
  ...
  const formatFriendlyDate = (dateStr: string) => {
    ...
    const localTodayStr = `${yStr}-${mStr}-${dStr}`;
    ...
    const localTomorrowStr = `${tmY}-${tmM}-${tmD}`;

    if (dateStr === localTodayStr) {
      return language === 'id' ? 'Hari Ini' : 'Today';
    } else if (dateStr === localTomorrowStr) {
      return language === 'id' ? 'Besok' : 'Tomorrow';
    } else {
      const parts = dateStr.split('-');
      if (parts.length === 3) {
        // Safe local parsing avoiding UTC shift
  ...
  const upcomingCombinedItems = useMemo(() => {
    const today = new Date();
    // Use local date at midnight to prevent timezone issues
    ...
    // Add local tasks starting from today
    tasks.forEach(task => {
    ...
    return items
      ...
      .slice(0, 5); // display top 5 upcoming items
  }, [tasks]);
                ...
                return (
                  <motion.div
                    ...
                    animate={{ 
                      opacity: 1, 
                      x: 0,
                      backgroundColor: item.completed ? '#141516' : '#0f1011'
                    }}
                    exit={{ opacity: 0, x: 10 }}
                    transition={{ duration: 0.2 }}
                    className={`p-4 flex items-center justify-between group transition-colors relative border-l-2 ${
                      item.completed ? 'border-ink-tertiary' : 'hover:bg-surface-2 border-transparent hover:border-accent'
                    }`}
                  >
                    <div className="flex items-center gap-4 relative z-10">
                      <button
                        ...
                        className={`w-6 h-6 rounded-md border flex items-center justify-center transition-all ${
                          item.completed ? 'bg-accent border-accent text-white shadow-glow-accent scale-110' : 'border-hairline-strong bg-surface-1 text-transparent hover:border-accent hover:scale-105'
                        }`}
                      >
                        <motion.div
                          initial={false}
                      ...
                      <div>
                        <div className={`text-body-sm font-bold transition-all relative inline-block ${item.completed ? 'text-ink-tertiary' : 'text-ink'}`}>
                          {item.title}
                          <motion.div 
    ...
    <div className="relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 lg:sticky lg:top-24 h-fit space-y-6">
          ...
          {/* Upcoming Unified Calendar & Task Agenda */}
          <div className="bg-surface-1 p-6 rounded-lg shadow-card border border-hairline space-y-4">
            <div className="flex items-center justify-between border-b border-hairline pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-[11px] font-bold text-ink uppercase tracking-wider">
                  {language === 'id' ? 'Agenda Mendatang' : 'Upcoming Agenda'}
                </h3>
              </div>
              <span className="text-[10px] bg-surface-2 text-ink-tertiary px-2 py-0.5 rounded-full font-mono font-bold">
                {upcomingCombinedItems.length} {language === 'id' ? 'Mendatang' : 'Upcoming'}
              </span>
            </div>

            <div className="space-y-3">
              {upcomingCombinedItems.length === 0 ? (
                <div className="text-center py-6 text-ink-tertiary text-xs italic">
                  {language === 'id' ? 'Tidak ada agenda mendatang' : 'No upcoming agenda'}
                </div>
              ) : (
                upcomingCombinedItems.map((item) => {
                  return (
                    <div 
                      key={item.id}
                      className="p-3 rounded-lg border flex flex-col gap-1 text-xs transition-all relative overflow-hidden bg-surface-2 border-hairline hover:border-hairline-strong"
                    >
                      <div className="flex items-center justify-between gap-2">
                        <span className={`font-bold truncate text-heading-xs ${
                          item.completed ? 'text-ink-tertiary line-through' : 'text-ink'
                        }`}>
                          {item.title}
                        </span>
                        <span className="text-[9px] font-mono shrink-0 px-1.5 py-0.5 rounded-md uppercase tracking-wider bg-surface-1 font-bold text-ink-tertiary border border-hairline/40">
                          {language === 'id' ? 'Tugas' : 'Task'}
                        </span>
                      </div>
                      <div className="flex items-center gap-3 text-[10px] text-ink-tertiary font-mono">
                        <span className="bg-accent/10 text-accent px-1.5 py-0.5 rounded-md font-bold">
